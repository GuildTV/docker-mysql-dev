...
if [ -e "/example.sql" ] && [ -z "$EXISTS" ]; then    
  ...
  mysqladmin -u root -p$MYSQL_ROOT_PASSWORD create "Test_$MYSQL_USERNAME"
  ...
  `mysql -u root -p$MYSQL_ROOT_PASSWORD -e "\
      ...
      FLUSH PRIVILEGES; "`
  `mysql -u root -p$MYSQL_ROOT_PASSWORD -e "\
      GRANT ALL PRIVILEGES ON "Test_$MYSQL_USERNAME.*" TO '$MYSQL_USERNAME'@'%' \
      IDENTIFIED BY '$MYSQL_PASSWORD' WITH GRANT OPTION; \
      FLUSH PRIVILEGES; "`

  # Import the SQL into new database
  `mysql -u root -p$MYSQL_ROOT_PASSWORD "$MYSQL_USERNAME" < /example.sql`
  `mysql -u root -p$MYSQL_ROOT_PASSWORD "Test_$MYSQL_USERNAME" < /example.sql`
